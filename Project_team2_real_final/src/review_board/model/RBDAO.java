...
public class RBDAO {
	...
	public void insertBoard(RBBean bean) {
		getCon();
		...
		try {
			...
			String refsql = "select max(ref) from review";
			// 쿼리 실행 객체
	...
	public RBBean getOneBoard(int num) {
		// 리턴타입 선언
		...
		try {
			// 조회수 증가 커리
			String readsql = "update review set readcount=readcount+1 where num=?";
	...
	// 작성글을 바로 보기 위한 메서드
	public void getOneBoard1(RBBean bean) {
		getCon();

		try {
			// 고유값num을 받아오기 위한 작업
			String sql = "select num, subject, ref from review where writer=? and password=? and subject=? order by num desc";
			pstmt = conn.prepareStatement(sql);
	...
	// 답변글이 저장되는 메서드
	public void reWriteBoard(RBBean bean) {
		...
		try {
			...
			// 기존 글변경
			String levelsql = "update review set re_step=re_step+1 where ref=? and re_step > ?";
			// 쿼리 삽입 객체 선언
	...
	public RBBean getOneUpdateBoard(int num) {
		// 리턴타입 선언
		RBBean bean = new RBBean();
		getCon();
	...
	public String getPass(int num) {
		...
		try {
			// 쿼리 준비
			String sql = "select password from review where num=?";
			// 쿼리 실행할 객체 선언
			pstmt = conn.prepareStatement(sql);
			pstmt.setInt(1, num);
			// 쿼리 실행후 결과를 리턴
	...
	// 하나의 게시글을 수정하는 메서드
	public void updateBoard(RBBean bean) {
		getCon();
	 ...
	 */
		...
		public ArrayList<RBBean> getBoardList(HashMap<String, Object> listOpt) {
		getCon();

		ArrayList<RBBean> list = new ArrayList<RBBean>();
		...
		try {
			...
			if (opt == null) {
				...
				sql = "select * from (select A.*,Rownum Rnum from(select * from review order by ref desc,re_step asc)A)"
						+ "where Rnum >= ? and Rnum <= ?";
			...
			{
				sql = "select * from (select A.*,Rownum Rnum from(select * from review where subject like ? "
			...
			{
				sql = "select * from (select A.*,Rownum Rnum from(select * from review where content like ? "
						+ "order by ref desc,re_step asc)A)" + "where Rnum >= ? and Rnum <= ?";
			...
			{
				sql = "select * from (select A.*,Rownum Rnum from(select * from review where subject like ? or content like ? "
						+ "order by ref desc,re_step asc)A)" + "where Rnum >= ? and Rnum <= ?";
			...
			{
				sql = "select * from (select A.*,Rownum Rnum from(select * from review where writer like ? "
						+ "order by ref desc,re_step asc)A)" + "where Rnum >= ? and Rnum <= ?";
...
*/
	// 회원의 문의내역
	public ArrayList<RBBean> getBoardMemberList(HashMap<String, Object> listOpt){
		getCon();
		ArrayList<RBBean> list = new ArrayList<RBBean>();
    	...
    	try {
    		if (id != null) {
    			...
    			*/
    			...
    			sql = "select * from (select A.*,Rownum Rnum from(select num,subject,writer,reg_date from review a ," 
    					+ "(select ref from review where id = ?) b " 
	...
	// 회원의 문의내역2
		public ArrayList<RBBean> getBoardMemberList2(HashMap<String, Object> listOpt){
			getCon();
			ArrayList<RBBean> list = new ArrayList<RBBean>();
	    	...
	    	try {
	    		if (id != null) {
	    			...
	    			sql = "select * from (select A.*,Rownum Rnum from(select num,id,writer,email,subject,password,reg_date,a.ref,re_step,re_level,readcount,content from review a ," 
	    					+ "(select ref from review where id = ?) b " 
	    					+ "where a.ref = b.ref order by a.ref desc,re_step)A ) where Rnum >= ? and Rnum <= ?";
	...
	public int getBoardListCount(HashMap<String, Object> listOpt) {
		...
		try {
			...
			if (opt == null) // 전체글의 개수
			{
				...
				sql = "select count(*) from review where subject like ?";
				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1, '%' + condition + '%');

			} else if (opt.equals("1")) { // 내용으로 검색한 글의 개수

				sql = "select count(*) from review where content like ?";
				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1, '%' + condition + '%');

			} else if (opt.equals("2")) // 제목+내용으로 검색한 글의 개수
			{
				sql = "select count(*) from review where subject like ? or content like ?";
				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1, '%' + condition + '%');
				pstmt.setString(2, '%' + condition + '%');
			...
			{
				sql = "select count(*) from review where writer like ?";
				pstmt = conn.prepareStatement(sql);
				pstmt.setString(1, '%' + condition + '%');
