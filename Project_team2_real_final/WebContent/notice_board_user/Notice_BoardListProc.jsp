...
<div class="content" align="center">
    ...
    <br>
    <table width="1000">		
		<tr height="40" style="background-color:#E6E6E6; font-weight: 700;">
        ...
        <%
        	...
        	<tr height="40">
				...
				<td width="320" align="left">
					<a href="BoardInfo.jsp?notice_num=<%=bean.getNotice_num() %>" style="text-decoration:none">
		...
		<tr height="40" style="border: none;">
			<td align="right" colspan="5">
				<input type="button" value="글 전체 보기" onclick="location.href='Notice_BoardList.jsp'">
	...
	<%
		if(listCount > 0){
			...
			if(startPage > 10){
	%>
			<a href="BoardListProc.jsp?pageNum=<%=startPage-10 %>" style="text-decoration:none">[이전]</a>
	<%
			}
			// 페이징 처리
			for(int i=startPage;i<=endPage;i++){
	%>
			<a href="BoardListProc.jsp?pageNum=<%=i %>&opt=<%=opt %>&condition=<%=condition %>" style="text-decoration:none">[<%=i %>]</a>
	<%
			}
			// 다음이라는 링크를  만들건지 파악
			if(endPage < pageCount){
	%>
			<a href="BoardListProc.jsp?pageNum=<%=startPage+10 %>" style="text-decoration:none">[다음]</a>
	<%
			}
		}
	%>
    ...
    <!--  검색 부분 -->
    <br>
        <form action="Notice_BoardListProc.jsp">
