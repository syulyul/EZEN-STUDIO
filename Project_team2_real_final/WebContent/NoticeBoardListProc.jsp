...
<div class="content" align="center">
    ...
    <table width="1000">		
        ...
        %>
        	<tr height="40">
				...
				<td width="320" align="left">
					<a href="NoticeBoardInfo.jsp?num=<%=bean.getNotice_num() %>" style="text-decoration:none">
				...
				<td width="100" align="center"><%=bean.getNotice_Writer() %></td>
				<td width="150" align="center"><%=bean.getNotice_reg_date() %></td>
				<td width="80" align="center"><%=bean.getNotice_readcount() %></td>
			</tr>	
		<%
			}
		%>
		<tr height="80" style="border: none;">
			<td align="right" colspan="5">
				...
				<input type="button" value="글쓰기" onclick="location.href='NoticeBoardWriteForm_admin.jsp'">
			</td>
		</tr>	
        </table>
	...
	<%
		if(listCount > 0){
			...
			if(startPage > 10){
	%>
			<a href="NoticeBoardListProc.jsp?pageNum=<%=startPage-10 %>" style="text-decoration:none">[이전]</a>
	<%
			}
			// 페이징 처리
			for(int i=startPage;i<=endPage;i++){
	%>
			<a href="NoticeBoardListProc.jsp?pageNum=<%=i %>&opt=<%=opt %>&condition=<%=condition %>" style="text-decoration:none">[<%=i %>]</a>
	<%
			}
			// 다음이라는 링크를  만들건지 파악
			if(endPage < pageCount){
	%>
			<a href="NoticeBoardListProc.jsp?pageNum=<%=startPage+10 %>" style="text-decoration:none">[다음]</a>
	<%
			}
		}
	%>
    ...
    <!--  검색 부분 -->
    <br>
        <form action="NoticeBoardListProc.jsp?opt=<%=opt%>">
