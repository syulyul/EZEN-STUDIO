...
<form action="index.jsp?pageChange=multiDelete_admin3.jsp" method="post" id="multidelete3">
	...
	<%
		if(listCount > 0){
			...
			if(startPage > 10){
	%>
			<a href="index.jsp?pageChange=BoardListProc_admin.jsp?pageNum=<%=startPage-10 %>" style="text-decoration:none; color:gray">[이전]</a>
	<%
			}
			// 페이징 처리
			for(int i=startPage;i<=endPage;i++){
	%>
			<a href="index.jsp?pageChange=BoardListProc_admin.jsp?pageNum=<%=i %>&opt=<%=opt %>&condition=<%=condition %>" style="text-decoration:none; color:gray">[<%=i %>]</a>
	<%
			}
			// 다음이라는 링크를  만들건지 파악
			if(endPage < pageCount){
	%>
			<a href="index.jsp?pageChange=BoardListProc_admin.jsp?pageNum=<%=startPage+10 %>" style="text-decoration:none; color:gray">[다음]</a>
	<%
			}
		}
	%>
    ...
    <br><br>
    <!--  검색 부분 -->
        <form action="index.jsp?pageChange=BoardListProc_admin.jsp?opt=<%=opt%>" method="post">
