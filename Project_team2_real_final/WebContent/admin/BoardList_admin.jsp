...
<h2>문의하기</h2>
<form action="index.jsp?pageChange=multiDelete_admin3.jsp" method="post" id="multidelete3">
...
<%-- 	
    ...
    <table width="1000">		
        ...
        %>
        	<tr height="40">
				...
				<td width="400" align="left">
					<a href="index.jsp?pageChange=BoardInfo_admin.jsp&num=<%=bean.getNum() %>" style="text-decoration:none">
		...
		<tr height="80" style="border: none;">
			<td align="right" colspan="7">
				...
				<!-- <input type="button" value="글쓰기" onclick="location.href='index.jsp?pageChange=BoardWriteForm_admin.jsp'"> -->
			</td>
		</tr>	
        </table>
	...
	<%
		if(listCount > 0){
			...
			if(startPage > 10){
	%>
			<a href="index.jsp?pageChange=BoardList_admin.jsp&pageNum=<%=startPage-10 %>" style="text-decoration:none; color:gray">[이전]</a>
	<%
			}
			// 페이징 처리
			for(int i=startPage;i<=endPage;i++){
	%>
			<a href="index.jsp?pageChange=BoardList_admin.jsp&pageNum=<%=i %>" style="text-decoration:none; color:gray">[<%=i %>]</a>
	<%
			}
			// 다음이라는 링크를  만들건지 파악
			if(endPage < pageCount){
	%>			
			<a href="index.jsp?pageChange=BoardList_admin.jsp&pageNum=<%=startPage+10 %>" style="text-decoration:none; color:gray">[다음]</a>
	<%
			}
		}
	%>
    ...
    <!--  검색 부분 -->
        ...
        <form action="index.jsp?pageChange=BoardListProc_admin.jsp" method="post">
