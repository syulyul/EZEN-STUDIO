...
<div class="content" align="center">
	...
	<%
		if(listCount > 0){
			...
			if(startPage > 10){
	%>
			<a href="index.jsp?pageChange=MemberListProc_admin.jsp&pageNum=<%=startPage-10 %>" style="text-decoration:none; color:gray">[이전]</a>
	<%
			}
			// 페이징 처리
			for(int i=startPage;i<=endPage;i++){
	%>
			<a href="index.jsp?pageChange=MemberListProc_admin.jsp&pageNum=<%=i %>&opt=<%=opt %>&condition=<%=condition %>" style="text-decoration:none; color:gray">[<%=i %>]</a>
	<%
			}
			// 다음이라는 링크를  만들건지 파악
			if(endPage < pageCount){
	%>
			<a href="index.jsp?pageChange=MemberListProc_admin.jsp&pageNum=<%=startPage+10 %>" style="text-decoration:none; color:gray">[다음]</a>
	<%
			}
		}
	%>
