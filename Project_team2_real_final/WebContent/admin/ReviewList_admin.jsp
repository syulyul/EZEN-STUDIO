...
<%-- 	
	...
	<%
		if(listCount > 0){
			...
			if(startPage > 10){
	%>
			<a href="index.jsp?pageChange=ReviewList_admin.jsp&pageNum=<%=startPage-10 %>" style="text-decoration:none; color:gray">[이전]</a>
	<%
			}
			// 페이징 처리
			for(int i=startPage;i<=endPage;i++){
	%>
			<a href="index.jsp?pageChange=ReviewList_admin.jsp&pageNum=<%=i %>" style="text-decoration:none; color:gray">[<%=i %>]</a>
	<%
			}
			// 다음이라는 링크를  만들건지 파악
			if(endPage < pageCount){
	%>			
			<a href="index.jsp?pageChange=ReviewList_admin.jsp&pageNum=<%=startPage+10 %>" style="text-decoration:none; color:gray">[다음]</a>
	<%
			}
		}
	%>
    ...
    <!--  검색 부분 -->
        ...
        <form action="index.jsp?pageChange=ReviewListProc_admin.jsp" method="post">
