		...
		<table width="600">
			...
			<tr height="60">
				<td align="center" colspan="4">
					...
					onclick="location.href='index.jsp?pageChange=BoardReWriteForm_admin.jsp?num=<%=bean.getNum()%>&ref=<%=bean.getRef() %>&re_step=<%=bean.getRe_step()%>&re_level=<%=bean.getRe_level()%>'">&nbsp;&nbsp;
					<%-- <input type="button" value="수정하기" onclick="location.href='index.jsp?pageChange=BoardUpdateForm_admin.jsp?num=<%=bean.getNum()%>'">&nbsp;&nbsp; --%>
					<input type="button" value="목록보기" onclick="location.href='index.jsp?pageChange=BoardList_admin.jsp?num=<%=bean.getNum()%>'">
				</td>
			</tr>
		</table>
</div>
